...
Multi-tenancy is **optional** and off by default. Set `MULTI_TENANCY=true` to
enable it; otherwise the server behaves exactly as before, running every
request and background job as a single implicit tenant (`default`).

When enabled:

- **Identity** = the `(Authorization key, X-Session-Id)` pair. The auth key is
  sent as today (`Authorization: Bearer <MCP_API_KEY>` or `X-API-Key`); the
  tenant additionally sends a **static** `X-Session-Id` header that is stable
  across reconnects. (The MCP `Mcp-Session-Id` header is auto-generated by the
  SDK after `initialize` and cannot be set ahead of connect, so a dedicated
  client-controlled header is used instead.)
- **Auto-provisioning, no admin.** The first time a `(key, session_id)` pair is
  seen, a `tenants` row is created automatically and an opaque, stable
  `tenant_id` is derived from it (the raw key is never stored, only its
  SHA-256 hash). No signup, no admin UI.
- **Data scoping.** Every tenant-owned table carries a `tenant_id` column and
  every query is filtered by the tenant resolved from the request context.
  Name/natural-key uniqueness becomes per-tenant, so two tenants can reuse the
  same contact email, template name, mailbox, chat, or external object id.
- **Channel config stays global.** WhatsApp / Threads / IMAP gateway URLs and
  tokens come from env and are shared; only the *data* they produce is scoped
  per tenant.
- **Public routes** (`/t`, `/o`, `/export`, `/u`, WhatsApp webhook) carry no
  session header. They resolve the owning tenant from the globally-unique
  random code/id on the matched row.
- **Background jobs** (scheduler, campaign sends) record the owning tenant on
  each task and re-inject it into context before running tenant-scoped work.

A client connects with, e.g.:

```
Authorization: Bearer <MCP_API_KEY>
X-Session-Id: <static-per-tenant-id>
```
...
| `MULTI_TENANCY` | no | `false` | When `true`, scope all data per tenant. Tenants are auto-provisioned (no admin) from the `(Authorization key, X-Session-Id)` pair on first connect; each tenant presents a static `X-Session-Id` header. When `false`, the server runs as a single implicit tenant (original behavior). See [Multi-tenancy](#multi-tenancy). |
